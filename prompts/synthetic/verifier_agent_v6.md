...
- CONTRADICTED requires positive captions from that audio to prove every changed proposition objectively incompatible with the claim.
- Reject contradictions based primarily on subjective intensity, manner, emotion, or quality, including gentle versus aggressive, loud versus quiet, or pleasant versus harsh.
- Prefer objectively checkable changes involving event or action occurrence, object or action identity, explicit count, direction, temporal order, or physical state.
- When supporting_caption_phrases contains multiple items, each must come from the sole evidence audio, describe the same coherent event, and appear in contradiction_basis for a contradicted claim.
